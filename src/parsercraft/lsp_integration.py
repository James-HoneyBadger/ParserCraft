...
from .lsp_advanced import (
    CodeFormatter,
    RefactoringEngine,
    SemanticHighlighter,
)
from .debug_adapter import DebugAdapter, Debugger
from .parser_generator import ASTNode
...
class LSPFeaturesIntegration:
    ...
    def _handle_rename_refactoring(
        ...
        edits = self.refactoring_engine.rename(
            uri, old_name, new_name, position
        )
    ...
    def handle_rename(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self._handle_rename_refactoring(
            {"oldName": "", "newName": new_name, "position": position},
            uri,
        )
    ...
    def handle_formatting(self, request: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Format document
        edits = self.code_formatter.format_document(
            uri,
            indent_size=options.get("tabSize", 4),
            use_tabs=options.get("insertSpaces", False),
        )
    ...
    def handle_range_formatting(
        ...
        # Format range
        edits = self.code_formatter.format_range(
            uri,
    ...
    def handle_semantic_tokens_full(
        ...
        if uri in self.highlight_cache:
            ...
            # Generate tokens
            tokens = self.semantic_highlighter.highlight_document(uri)
    ...
    def handle_semantic_tokens_range(
        ...
        tokens = self.semantic_highlighter.highlight_range(
    ...
    def handle_debug_start(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        session_id = self.debug_adapter.init_session(program)
    ...
    def handle_debug_stop(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if session_id:
            self.debug_adapter.end_session(session_id)
    ...
    def register_all_features(self) -> None:
        ...
        for req_type, handler in self.request_handlers.items():
            self.lsp_server.register_request_handler(req_type, handler)

        for notif_type, handler in self.notification_handlers.items():
            self.lsp_server.register_notification_handler(notif_type, handler)
