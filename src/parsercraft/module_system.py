...
class ModuleExport:
    ...
    def is_accessible_from(self, module: "Module") -> bool:
        ...
        if self.visibility == ModuleVisibility.PRIVATE:
            return module.name == self.defined_at  # Only accessible within same module
...
class ModuleManager:
    ...
    def load_with_dependencies(self, module_name: str) -> Dict[str, Module]:
        ...
        while to_process:
            ...
            except (ModuleNotFoundError, ModuleLoadError) as e:
                print(f"Warning: {e}")
                if not any(d.optional for d in [dep for dep in module.dependencies
                                                if dep.module_name == current]):
                    raise
